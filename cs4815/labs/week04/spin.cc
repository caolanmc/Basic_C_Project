...
processMenuEvents(int option)
{
    switch(option)
    {
        ...
        case INCREASE : speed+=.1; break;
        case DECREASE : speed+=-.1; break;
        case HALF : speed*=.5; break;
...
createGLUTMenus()
{
  int menu,submenuDir,submenuSpeed,submenuCol;

  submenuCol=glutCreateMenu(processMenuEvents);
  glutAddMenuEntry("Red",RED);
  glutAddMenuEntry("Blue",BLUE);
  glutAddMenuEntry("Green",GREEN);
  glutAddMenuEntry("White",WHITE);

  submenuDir=glutCreateMenu(processMenuEvents);
  glutAddMenuEntry("Reverse",REVERSE);
  glutAddMenuEntry("Clockwise",CLOCKWISE);
  glutAddMenuEntry("Anti-Clockwise",ANTICLOCKWISE);

  submenuSpeed=glutCreateMenu(processMenuEvents);
  ...
  glutAddSubMenu("Direction",submenuDir);
  glutAddSubMenu("Speed",submenuSpeed);
  glutAddSubMenu("Colour",submenuCol);
